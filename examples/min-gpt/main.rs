...
fn causal_self_attention(
    ...
    nn::func_t(
        move |xs: &Tensor, train| {
            ...
            let att =
                att.masked_fill(&mask.i((.., .., ..sz_t, ..sz_t)).eq(0.), std::f64::NEG_INFINITY);
...
fn block(p: &nn::Path, cfg: Config) -> impl ModuleT<Input = Tensor, Output = Tensor> {
    ...
    nn::func_t(
        move |xs: &Tensor, train| {
            ...
            let ys =
                xs.apply(&ln2).apply(&lin1).gelu().apply(&lin2).dropout(cfg.resid_pdrop, train);
            xs + ys
        },
        |m, xs, ys, d, bs| nn::batch_accuracy_for_logits(m, xs, ys, d, bs),
    )
...
fn gpt(p: &nn::Path, cfg: Config) -> impl ModuleT<Input = Tensor, Output = Tensor> {
...
fn sample(
    data: &TextData,
    gpt: &impl ModuleT<Input = Tensor, Output = Tensor>,
    input: Tensor,
) -> String {
    let mut input = input;
