...
let write_wrapper funcs filename ~fallible =
      ...
      Map.iteri funcs ~f:(fun ~key:exported_name ~data:(func : Func.t) ->
          ...
          let returns =
            ...
            | n ->
                ...
                |> String.concat ~sep:", " |> Printf.sprintf "(%s)"
          in
          let returns = if fallible then "Ok(" ^ returns ^ ")" else returns in
          pm "" ;
          let prefix = if fallible then "f_" else "" in
          pm "    pub fn %s%s%s(" prefix rust_name (Func.type_parameters func) ;
          let self, rust_args_list = Func.rust_args_list func in
          pm "        %s" rust_args_list ;
          pm "    )%s {" (Func.rust_return_type func ~fallible) ;
          pm "        let mut c_tensors = [std::ptr::null_mut(); %d];"
            func.returns ;
          pm "        unsafe_torch%s!({" (if fallible then "_err" else "") ;
...
let run ~yaml_filename ~cpp_filename ~ffi_filename ~wrapper_filename
  ...
  write_wrapper funcs wrapper_filename ~fallible:false ;
  write_wrapper funcs fallible_wrapper_filename ~fallible:true
