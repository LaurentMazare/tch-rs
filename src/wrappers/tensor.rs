...
impl Tensor {
    ...
    /// returned otherwise.
    pub fn f_double_value(&self, idx: &[i64]) -> Result<f64> {
        Ok(unsafe_torch_err!({
    ...
    /// returned otherwise.
    pub fn f_int64_value(&self, idx: &[i64]) -> Result<i64> {
        Ok(unsafe_torch_err!({
    ...
    pub fn f_copy_data_u8(&self, dst: &mut [u8], numel: usize) -> Result<()> {
    ...
    /// Copies `numel` elements from `self` to `dst`.
    pub fn f_copy_data<T: kind::T>(&self, dst: &mut [T], numel: usize) -> Result<()> {
        ensure!(
            T::KIND == self.kind(),
            "incoherent elt kind, {:?} != {:?}",
            self.kind(),
    ...
    pub fn f_of_slice<T: kind::T>(data: &[T]) -> Result<Tensor> {
    ...
    pub fn f_of_data_size(data: &[u8], size: &[i64], kind: Kind) -> Result<Tensor> {
        let data = data.as_ptr() as *const c_void;
    ...
    pub fn f_get(&self, index: i64) -> Result<Tensor> {
    ...
    pub fn f_copy_(&mut self, src: &Tensor) -> Result<()> {
    ...
    pub fn load<T: AsRef<Path>>(path: T) -> Result<Tensor> {
    ...
    ///
    /// The file format is the same as the one used by the PyTorch C++ API.
    pub fn load_multi<T: AsRef<Path>>(path: T) -> Result<Vec<(String, Tensor)>> {
        let path = path_to_cstring(path)?;
    ...
    /// large tensors.
    pub fn to_string(&self, lw: i64) -> Result<String> {
