...
pub fn load_hwc<T: AsRef<Path>>(path: T) -> Result<Tensor> {
...
pub fn save_hwc<T: AsRef<Path>>(t: &Tensor, path: T) -> Result<()> {
    let path = path_to_cstring(path)?;
...
/// Expects a tensor of shape [width, height, channels].
/// On success returns a tensor of shape [width, height, channels].
pub fn resize_hwc(t: &Tensor, out_w: i64, out_h: i64) -> Result<Tensor> {
