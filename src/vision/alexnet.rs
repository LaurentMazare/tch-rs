...
use crate::nn;
use crate::nn::{Conv2D, Linear, ModuleT, SequentialT};
use crate::Tensor;
...
fn classifier(p: nn::Path, nclasses: i64) -> impl ModuleT {
    SequentialT::new()
        ...
        .add(Linear::new(&p / "1", 256 * 6 * 6, 4096, Default::default()))
        .add_fn(|xs| xs.relu())
        .add_fn_t(|xs, train| xs.dropout(0.5, train))
        .add(Linear::new(&p / "4", 4096, 4096, Default::default()))
        .add_fn(|xs| xs.relu())
        .add(Linear::new(&p / "6", 4096, nclasses, Default::default()))
}
