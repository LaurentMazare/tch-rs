...
fn read_file(filename: &std::path::Path) -> Result<(Tensor, Tensor)> {
    ...
    for index in 0..SAMPLES_PER_FILE {
        ...
        labels.narrow(0, index, 1).copy_(&content.narrow(0, content_offset, 1));
        images.narrow(0, index, 1)
            .copy_(&content.narrow(0, 1+content_offset, BYTES_PER_IMAGE).view(&[1, C, H, W]));
    }
...
pub fn load_dir(dir: &std::path::Path) -> Result<Dataset> {
    ...
    let train_images_and_labels =
      [ "data_batch_1.bin",
        ...
        "data_batch_4.bin",
        "data_batch_5.bin",
      ].iter().map(|x| read_file(&dir.join(x))).collect::<Result<Vec<_>>>()?;
