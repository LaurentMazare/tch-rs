...
pub fn save<T: AsRef<Path>>(t: &Tensor, path: T) -> Result<()> {
...
pub fn resize(t: &Tensor, out_w: i64, out_h: i64) -> Result<Tensor> {
    Ok(hwc_to_chw(&resize_hwc(&chw_to_hwc(t), out_w, out_h)?))
}

pub fn resize_preserve_aspect_ratio_hwc(t: &Tensor, out_w: i64, out_h: i64) -> Result<Tensor> {
...
///
/// This expects as input a tensor of shape [channel, height, width] and returns
pub fn resize_preserve_aspect_ratio(t: &Tensor, out_w: i64, out_h: i64) -> Result<Tensor> {
...
pub fn load_and_resize<T: AsRef<Path>>(path: T, out_w: i64, out_h: i64) -> Result<Tensor> {
    let tensor = load_hwc(path)?;
    resize_preserve_aspect_ratio_hwc(&tensor, out_w, out_h)
}

fn visit_dirs(dir: &Path, files: &mut Vec<std::fs::DirEntry>) -> Result<()> {
    if dir.is_dir() {
...
pub fn load_dir<T: AsRef<Path>>(path: T, out_w: i64, out_h: i64) -> Result<Tensor> {
