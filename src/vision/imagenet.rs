...
pub const CLASS_COUNT_21K: i64 = 21843;

pub const CLASSES_21K: [&str; 21843] = [
    "organism, being",
    "benthos",
    "heterotroph",
    "cell",
...
/// Returns the top k classes as well as the associated scores for the imagenet21k labels.
pub fn top_21k(tensor: &Tensor, k: i64) -> Vec<(f64, String)> {
    let tensor = match tensor.size().as_slice() {
        [CLASS_COUNT_21K] => tensor.shallow_clone(),
        [1, CLASS_COUNT_21K] => tensor.view((CLASS_COUNT_21K,)),
        [1, 1, CLASS_COUNT_21K] => tensor.view((CLASS_COUNT_21K,)),
        _ => panic!("unexpected tensor shape {tensor:?}"),
    };
    let (values, indexes) = tensor.topk(k, 0, true, true);
    let values = Vec::<f64>::try_from(values).unwrap();
    ...
    values
        .iter()
        .zip(indexes.iter())
        .map(|(&value, &index)| (value, CLASSES_21K[index as usize].to_owned()))
        .collect()
}